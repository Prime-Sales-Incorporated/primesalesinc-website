...
/* ─────────────────────────────────────────────
   ...
   NOT inside the component (avoids re-injection
   on every render / state change)
───────────────────────────────────────────── */
if (typeof document !== "undefined" && !document.getElementById("hh-styles")) {
  const s = document.createElement("style");
  s.id = "hh-styles";
  s.textContent = `
    /* Use opacity + translate only — GPU-composited, no layout reflow */
...
/* ─────────────────────────────────────────────
   SUB-COMPONENTS — memoised to avoid re-renders
───────────────────────────────────────────── */
 ...
 * Single product tile.
 *
 * CHANGED: if prod.link is set, navigate directly to that path.
 * Otherwise fall back to /solutions with state (previous behaviour).
 */
...
function HeaderHome({ dark, setDark }) {
  ...
  /* stable callbacks — no recreation on each render */
  ...
  return (
    <nav className="absolute top-0 left-0 right-0 z-50 bg-black/10 backdrop-blur-sm py-4 px-6">
      <div className="flex justify-between items-center">
        ...
        {/* Dark toggle — desktop */}
        <div
          className="absolute top-7 right-5 hidden md:flex items-center justify-between w-12 h-6 p-1 rounded-full cursor-pointer bg-gray-800"
          onClick={toggleDark}
        >
          <FaMoon
            className={`text-yellow-300 text-xs ${dark ? "opacity-100" : "opacity-50"} transition-opacity`}
          />
          <FaSun
            className={`text-yellow-400 text-xs ${dark ? "opacity-50" : "opacity-100"} transition-opacity`}
          />
          <div
            className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full shadow transform transition-transform duration-300 ${dark ? "translate-x-6" : "translate-x-0"}`}
          />
        </div>
            ...
            <div className="flex flex-col">
              <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
                ...
                <div className="flex items-center gap-3">
                  <div
                    className="relative flex items-center justify-between w-12 h-6 p-1 rounded-full cursor-pointer bg-gray-700"
                    onClick={toggleDark}
                  >
                    <FaMoon
                      className={`text-yellow-300 text-xs ${dark ? "opacity-100" : "opacity-40"}`}
                    />
                    <FaSun
                      className={`text-yellow-400 text-xs ${dark ? "opacity-40" : "opacity-100"}`}
                    />
                    <div
                      className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full shadow transform transition-transform duration-300 ${dark ? "translate-x-6" : "translate-x-0"}`}
                    />
              ...
              <div className="flex flex-col divide-y divide-white/10">
                ...
                <button className="flex items-center justify-between px-5 py-4 text-sm font-medium hover:bg-white/5 transition-colors w-full text-left">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex flex-col divide-y divide-white/10">
                {activeCategory.products.map((prod, i) => (
                  <Link
                    key={i}
                    /* CHANGED: use prod.link if available, otherwise fall back */
